...
function consultarProveedores(req, res, next) {
  const { campo = 'empresa', filtro } = req.query;

  // Lista de campos permitidos para evitar inyecciones
  const camposPermitidos = ['empresa', 'direccion', 'descripcion'];

  let sql = '';
  let params = [];

  if (filtro && camposPermitidos.includes(campo)) {
    sql = `SELECT * FROM Proveedores WHERE ${campo} LIKE ?`;
    params.push(`%${filtro}%`);
  } else {
